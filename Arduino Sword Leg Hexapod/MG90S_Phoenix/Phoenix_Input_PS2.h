...
void InputController::Init(void)
{
 ...
 if(error == 0){
   ...
   DBGSerial.println("No controller found, check wiring, see readme.txt to enable debug. visit www.billporter.info for troubleshooting tips");
   
  else if(error == 2)
   DBGSerial.println("Controller found but not accepting commands. see readme.txt to enable debug. Visit www.billporter.info for troubleshooting tips");
   
  else if(error == 3)
   DBGSerial.println("Controller refusing to enter Pressures mode, may not support it. ");

   type = ps2x.readType(); 
     switch(type) {
       case 0:
        DBGSerial.println("Unknown Controller type");
       break;
       case 1:
        DBGSerial.println("DualShock Controller Found");
       break;
       case 2:
         DBGSerial.println("GuitarHero Controller Found");
       break;
     }
